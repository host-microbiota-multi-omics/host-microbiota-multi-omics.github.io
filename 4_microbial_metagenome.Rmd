...
## Functional annotation with DRAM

DRAM annotates MAGs by integrating multiple databases (KEGG, UniRef, PFAM, MEROPS, etc.) to assign gene-level
functions and pathway summaries. Running DRAM on the dereplicated, taxonomically classified bins yields genome-
scale metabolic profiles, gene presence/absence tables, and curated reports that are ready for downstream
comparative and ecological analyses. The rule will consume the bin FASTA files, run DRAM’s annotate/distill
pipeline, and produce summarized tables for interpretation and visualization.
